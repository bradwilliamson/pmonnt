    ...
    for &col in columns {
        match col {
            ...
            ProcessColumnId::Leader => {
                ...
                header.col(|ui| {
                    let _ = header_cell_button(app, ui, col, "Leader", &released_any);
                });
            }
            ...
            ProcessColumnId::CPU => {
                header.col(|ui| {
                    ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                        let cpu_label = if app.group_sort == GroupSort::CPU {
                            if app.sort_desc {
                                "v CPU"
                            } else {
                                "^ CPU"
            ...
            ProcessColumnId::Memory => {
                header.col(|ui| {
                    ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                        let mem_label = if app.group_sort == GroupSort::Memory {
                            if app.sort_desc {
                                "v Memory"
                            } else {
                                "^ Memory"
            ...
            ProcessColumnId::GPU => {
                header.col(|ui| {
                    ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                        let gpu_label = if app.group_sort == GroupSort::GPU {
                            if app.sort_desc {
                                "v GPU"
                            } else {
                                "^ GPU"
